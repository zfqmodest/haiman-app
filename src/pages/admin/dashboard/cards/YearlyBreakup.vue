<template>
  <VaCard>
    <VaCardTitle class="pb-0!">
      <h1 class="card-title text-secondary font-bold uppercase">{{ t('dashboard.cards.yearlyBreakup') }}</h1>
    </VaCardTitle>
...
// 创建国际化的图表数据
const localizedChartData = computed(() => {
  ...
  return {
    ...
    datasets: chartData.value.datasets.map((dataset) => ({
      ...dataset,
      label: t('dashboard.cards.yearlyBreakup'),
    })),
  }
})
